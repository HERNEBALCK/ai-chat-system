...
const sendMessage = () => {
    ...
    if(isAiTyping.value) { // AI处理信息中
        ElMessage(error('琪露诺正在头脑风暴，请耐心等等ww！'))
        return
    }
...
const startNewSession = (message) => {
    ...
    if(currentSession.value.sessionTitle === '新对话') {
        sessionParams.sessionTitle = `琪露诺AI助手 - ${new Date().toLocaleString()}`
    } else { // 历史会话
...
const startAIResponse = (sessionId, userMessage) => {
    // 防止重复发送
    if(isAiTyping.value) {
        ElMessage.error('琪露诺祈祷中···(bakabakabaka)')
        return
    }
    ...
    // 调用流式接口
    const ctrl = new AbortController() // 终止fectch请求
    fetchEventSource('/api/psychological-chat/stream', {
        ...
        onmessage: (event) => {
            ...
            if(ok && payload.data && payload.data.content) {
                ...
                // 错误回复提示
                handleError(payload.message || '琪露诺和大酱出去玩惹，请等会再过来喵！')
            }
        },
        onerror: (err) => {
            handleError(err.message || '琪露诺回复失败')
            throw err
        },
        onclose: () => {
            // 开始情绪分析
...
const handleError = (error) => {
    const aiMessage = messages.value[messages.value.length - 1]
    if(aiMessage) {
        aiMessage.content = '琪露诺跑出去玩惹，请等会再来！'
    }
    isAiTyping.value = false
    ElMessage.error('琪露诺跑出去玩惹，请等会再来！')
}
    ...
    <div class="consultation-container">
        <div class="sidebar">
            ...
            <div class="ai-assistant-info">
                ...
                <h3 class="assistant-name">琪露诺AI助手</h3>
        ...
        <div class="chat-main">
            ...
            <div class="chat-messages">
                ...
                <div class="message-item" v-for="msg in messages" :key="msg.id" :class="msg.senderType === 1 ? 'user-message' : 'ai-message'">
                    <div class="message-avatar">
                        <el-image v-if="msg.senderType === 1" :src="iconUrl2" style="height: 18px; width: 18px;"></el-image>
                        <el-image v-if="msg.senderType === 2" :src="iconUrl" style="height: 18px; width: 18px;"></el-image>
                    </div>
                    <div class="message-content">
                        <div class="message-bubble">
                            <!-- ai正在思考中 -->
